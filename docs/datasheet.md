...
The initial dataset contains known starting observations for each function. Each weekly round adds one new observed query/output pair per function. At the time of this datasheet, Week 9 results have been recorded and Week 10 has been submitted, so the observed dataset contains initial data plus Week 1 to Week 9 returns. Week 10 outputs are not yet available.
